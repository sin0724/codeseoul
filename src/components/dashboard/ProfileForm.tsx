...
import { Plus, Trash2, Check, ArrowUpCircle, ChevronDown, ChevronUp, Instagram } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-xl">
      ...
      <div className="border-t border-white/10 pt-4">
        <button
          ...
          className="flex items-center gap-2 w-full text-left text-sm font-mono text-white/50 hover:text-white/70 transition-colors py-2"
        >
          ...
          <div className="mt-4 space-y-6">
            ...
            <div className="rounded border border-white/10 bg-white/5 p-6 space-y-4">
              ...
              <div>
                ...
                <input value={beneficiaryName} onChange={(e) => setBeneficiaryName(e.target.value)} placeholder="e.g. WANG MING-CHEN" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.addressEnglish}</label>
                <input value={addressEnglish} onChange={(e) => setAddressEnglish(e.target.value)} placeholder="e.g. No.123, Section 1, Zhongxiao E. Rd., Taipei, Taiwan" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.phoneNumber}</label>
                <input value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="e.g. +886-2-12345678" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.bankName}</label>
                <input value={bankName} onChange={(e) => setBankName(e.target.value)} placeholder="e.g. CTBC Bank" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.swiftCode}</label>
                <input value={swiftCode} onChange={(e) => setSwiftCode(e.target.value.toUpperCase())} placeholder="e.g. CTCBTWTP" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.bankAddress}</label>
                <input value={bankAddress} onChange={(e) => setBankAddress(e.target.value)} placeholder="e.g. No.123, Xinyi Rd., Taipei, Taiwan" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.accountNumber}</label>
                <input value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} placeholder="e.g. 12345678901234" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
              <div>
                <label className="block text-sm text-white/80 mb-1 font-mono">{zhTW.iban}</label>
                <input value={iban} onChange={(e) => setIban(e.target.value.toUpperCase())} placeholder="e.g. TW1234567890123456789012 (if applicable)" className="w-full rounded border border-white/20 bg-black/50 px-4 py-2 font-mono text-white placeholder:text-white/40" />
              </div>
            </div>
          </div>
        )}
